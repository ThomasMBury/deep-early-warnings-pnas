...
filepath = 'data/ml_preds/{}/parsed/'.format(ml_number)
...
for tsid in tsid_vals:
    for null_number in [1,2]:
        ...
        df = df_ews_null[(df_ews_null['tsid']==tsid)&\
                         (df_ews_null['Null number']==null_number)
                         ]
...
fig.update_layout(
    ...
    width=600,
    height=600,
    title='ROC, Thermoacoustic, ML {}'.format(ml_number)
    )

fig.write_image('figures/figs_roc/roc_thermo_{}.png'.format('early' if bool_pred_early else 'late'))
