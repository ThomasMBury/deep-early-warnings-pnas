...
Null simulations in Hopf regime
Compute EWS
...
ah = 3 * sf  # control parameter final value
...
for j in range(numSims):
    ...
    data = {"tsid": (j + 1) * np.ones(len(t)), "Time": t, "x": x, "y": y}
...
# # Export individual residual files (in x) for training ML
# for i in np.arange(numSims)+1:
#     df_resids = df_ews.loc[i,'x'].reset_index()[['Time','Residuals']]
#     filepath='data/resids/cr_hopf_null_1500_resids_{}.csv'.format(i)
