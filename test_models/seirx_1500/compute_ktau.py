...
df_ews_forced.set_index(['tsid','var','Time'], inplace=True)

# Import EWS from null trajectories 
df_ews_null = pd.read_csv('data/ews/df_ews_null.csv')
df_ews_null.set_index(['tsid','var','Time'], inplace=True)
...
var_vals = df_ews_forced.index.unique(level='var')
...
df_ktau_forced = pd.concat(list_df).set_index(['tsid','var','Time'])
...
var_vals = df_ews_null.index.unique(level='var')
...
# Concatenate kendall tau dataframes
df_ktau_null = pd.concat(list_df).set_index(['tsid','var','Time'])
