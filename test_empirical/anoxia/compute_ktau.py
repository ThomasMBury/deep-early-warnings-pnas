...
df_ews_forced.set_index(["tsid", "Variable label", "Age [ka BP]"], inplace=True)

# Import EWS from null Dakos climate transitions
df_ews_null = pd.read_csv("data/ews/df_ews_null.csv")
df_ews_null.set_index(["tsid", "Variable label", "Age [ka BP]"], inplace=True)
...
for tsid in tsid_vals:
    for var in ["Mo", "U"]:
        ...
        series_ac = df_ews_forced.loc[tsid, var]["ac1"]
...
df_ktau_forced = pd.concat(list_df).set_index(["tsid", "Age [ka BP]"])
...
for tsid in tsid_vals:
    for var in ["Mo", "U"]:
        ...
        series_ac = df_ews_null.loc[tsid, var]["ac1"]
...
# Concatenate kendall tau dataframes
df_ktau_null = pd.concat(list_df).set_index(["tsid", "Age [ka BP]"])
