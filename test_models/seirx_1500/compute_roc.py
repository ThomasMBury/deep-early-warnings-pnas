...
Compute ROC data for EWS and ML predictions
...
df_ml_forced = pd.read_csv('data/ml_preds/df_ml_forced.csv')
df_ml_null = pd.read_csv('data/ml_preds/df_ml_null.csv')
...
#-------------------
# Bifurcation prediction counts for variable x
#-------------------
...
# Count each bifurcaiton choice
counts = df_ml_preds[
    (df_ml_preds['truth value']==1)&\
    (df_ml_preds['var']=='x')
    ]['fav_bif'].value_counts()
...
filepath = 'data/roc/df_bif_pred_counts_x_{}.csv'.format(
...
#-------------------
# Bifurcation prediction counts for variable I
#-------------------
...
# Export data on bifurcation prediction counts
filepath = 'data/roc/df_bif_pred_counts_I_{}.csv'.format(
    'early' if bool_pred_early else 'late')
...
#---------------------
# Compute ROC data for variable x
#–--------------------
...
indicator_vals = df_ml_preds[df_ml_preds['var']==var]['bif_prob']
truth_vals = df_ml_preds[df_ml_preds['var']==var]['truth value']
...
indicator_vals = df_ktau_preds[df_ktau_preds['var']==var]['ktau_variance']
truth_vals = df_ktau_preds[df_ktau_preds['var']==var]['truth value']
df_roc = roc_compute(truth_vals,indicator_vals)
df_roc['ews'] = 'Variance'
list_roc.append(df_roc)
...
indicator_vals = df_ktau_preds[df_ktau_preds['var']==var]['ktau_ac']
truth_vals = df_ktau_preds[df_ktau_preds['var']==var]['truth value']
df_roc = roc_compute(truth_vals,indicator_vals)
df_roc['ews'] = 'Lag-1 AC'
list_roc.append(df_roc)
...
# Export ROC data
filepath = 'data/roc/df_roc_seirx_x_{}.csv'.format(
    'early' if bool_pred_early else 'late')
...
fig.update_layout(
    ...
    width=600,
    height=600,
    title='ROC for SEIRx model - variable x',
    )

fig.write_image('figures/roc_x_{}.png'.format('early' if bool_pred_early else 'late'))
...
#---------------------
# Compute ROC data for variable I
#–--------------------
...
# Export ROC data
filepath = 'data/roc/df_roc_seirx_I_{}.csv'.format(
    'early' if bool_pred_early else 'late')
...
fig.update_layout(
    ...
    width=600,
    height=600,
    title='ROC for SEIRX model, variable I',
    )

fig.write_image('figures/roc_I_{}.png'.format('early' if bool_pred_early else 'late'))
