...
Get ensemble predictions from len500 DL classifiers for May fold data
...
for tsid in tsid_vals:
    ...
    series = (
        df_ews_forced[(df_ews_forced["tsid"] == tsid)]
        .set_index("Time")["residuals"]
        .dropna()
    )
    # apply classifer to last 500 points prior to transitios
    ...
    dl_preds_mean.to_csv(
        f"data/ml_preds/ensemble_trend_probs_cr_hopf_forced_{tsid}.csv",
    )
...
for tsid in tsid_vals:
    print(f"compute dl predictions for null time seires of tsid={tsid}")
    series = df_ews_null[(df_ews_null["tsid"] == tsid)].set_index("Time")["residuals"]
    ...
    # Export
    dl_preds_mean.to_csv(
        f"data/ml_preds/ensemble_trend_probs_cr_hopf_null_{tsid}.csv",
    )
