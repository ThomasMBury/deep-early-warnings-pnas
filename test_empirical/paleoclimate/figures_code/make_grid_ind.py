...
# ML model number
model500 = 'Protocol3_Jan21_len500'
model1500 = 'Protocol3_Jan21_len1500'
...
df_ml = pd.read_csv('../data/ml_preds/model_{}/parsed/df_ml_forced.csv'.format(model500))
df_ml['Time'] = df_ml['Age']


# Use 1500 ML prediction for tsid=3 (long)
df_ml_1500 = pd.read_csv('../data/ml_preds/model_{}/parsed/df_ml_forced.csv'.format(model1500))
df_ml_1500['Time'] = df_ml_1500['Age']
...
def make_grid_figure(tsid, letter_label, title):
    ...
    fig.update_xaxes(title={'text':'Age (yr BP)',
                            'standoff':5},
                     ticks="outside",
                     showline=True,
                     linewidth=1,
    ...
    fig.update_yaxes(title={'text':proxy_label,
                            'standoff':5,
                            },
                     row=1,col=1)
...
for tsid in list_tsid: 
    ...
    # Export as png
    fig.write_image('../figures/fig_grid_ind/ind_figs/img_{}.png'.format(tsid),
                    scale=2)  
