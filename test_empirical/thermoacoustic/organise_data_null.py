...
df_voltage = pd.read_csv(path_state+'control parameter/voltage_in_volt.txt',
                         header=None,
                         names=['Voltage (V)'],
                         )
...
for label in list_labels:
    ...
    df_state = pd.read_csv(path_state+filename,
                           delimiter='\t',
                           header=0,
                           names=['temp','pressure'])
...
# Export null data
df_nulls.to_csv('data/processed_data/df_nulls.csv',
                      index=False,
                      )
