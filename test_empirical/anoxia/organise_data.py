...
for id_val in id_vals:
    ...
    if id_val in id_vals_ms21:
        ...
        df_extract = df_ms21[
            (df_ms21["Age [ka BP]"] >= tmin) & (df_ms21["Age [ka BP]"] <= tmax)
        ].copy()
    ...
    if id_val in id_vals_ms66:
        ...
        df_extract = df_ms66[
            (df_ms66["Age [ka BP]"] >= tmin) & (df_ms66["Age [ka BP]"] <= tmax)
        ].copy()
        df_extract["ID"] = id_val
    ...
    if id_val in id_vals_64pe:
        ...
        df_extract = df_64pe[
            (df_64pe["Age [ka BP]"] >= tmin) & (df_64pe["Age [ka BP]"] <= tmax)
        ].copy()
        df_extract["ID"] = id_val
...
df_transition_data.to_csv("data/data_transitions.csv", index=False)
