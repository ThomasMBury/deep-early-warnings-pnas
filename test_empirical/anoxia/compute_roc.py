...
for tsid in tsid_vals:
    for var in ["U", "Mo"]:
        ...
        df = df_ews_forced[
            (df_ews_forced["tsid"] == tsid) & (df_ews_forced["Variable label"] == var)
        ]
        ...
        idx = np.round(
            np.linspace(0, len(df_ktau_forced_final) - 1, n_predictions)
        ).astype(int)
        list_df_ktau_preds.append(df_ktau_forced_final.iloc[idx])

        # ML forced trajectories
        idx = np.round(
            np.linspace(0, len(df_ml_forced_final) - 1, n_predictions)
        ).astype(int)
...
fig.write_image(
    "figures/figs_roc/roc_anoxia_{}.png".format("early" if bool_pred_early else "late")
)
