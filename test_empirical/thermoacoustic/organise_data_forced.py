...
path_bifparam = 'data/thermo_experiments/Rate dependent transitions/Control_parameter_variation/'
...
df_trans_times = pd.DataFrame(
    {'tsid':np.arange(1,20),
     'transition time (s)':list_trans_times
     }
    ).set_index('tsid')
...
for label in list_labels:
    ...
    df_state = pd.read_csv(path_state + filename_state,
                           header=0,
...
df_transitions.to_csv('data/processed_data/df_transitions.csv',
                      index=False,
                      )
