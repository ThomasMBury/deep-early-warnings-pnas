...
Get ensemble predictions from len1500 DL classifiers for thermoacoustic data
...
for tsid in tsid_vals[1:]:
    print(f"compute dl predictions for tsid={tsid}")
    series = df_ews_forced[(df_ews_forced["tsid"] == tsid)].set_index("Time (s)")[
        "residuals"
    ]
