...
def make_grid_figure(tsid, var_label, letter_label, title):
    ...
    ymin = df_ews_plot['State variable'].min()
    ymax = df_ews_plot['State variable'].max()
...
for tsid in list_tsid: 
    ...
    # Export as png
    fig.write_image('../figures/fig_grid_ind/ind_figs_mo_null/img_{}.png'.format(tsid),
                    scale=2)    
...
for tsid in list_tsid: 
    ...
    # Export as png
    fig.write_image('../figures/fig_grid_ind/ind_figs_u_null/img_{}.png'.format(tsid),
                    scale=2)    
    print('Exported for tsid = {}'.format(tsid))
