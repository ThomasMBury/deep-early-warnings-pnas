...
df = pd.read_csv('data/tripati2005/tripati2005_select.csv',
                 header=0,
                 names=['Age','Proxy'],
                 )
# Millions of years
df['Age'] = df['Age']*1e6
df = df[(df['Age']<=40e6)&(df['Age']>=32e6)].sort_values('Age',ascending=False).reset_index(drop=True)
...
df = pd.read_csv('data/gisp2/gisp2_temp_accum_alley2000.txt',
                 header=0,
                 names=['Age','Proxy','NA'],
...
df=df[['Age','Proxy']]
df['Age'] = df['Age']*1000
df = df[(df['Age']<=21000)&(df['Age']>=14600)].sort_values('Age',ascending=False).reset_index(drop=True)
# df.plot(x='Age',y='Proxy')
...
df = pd.read_csv('data/cariaco2000/cariaco2000_pc56_greyscale.txt',
                 header=1,
                 names=['Age','Proxy'],
                 delim_whitespace=True,
                 # nrows=1632,
                 )
df = df[(df['Age']<=12500)&(df['Age']>=11200)].sort_values('Age',ascending=False).reset_index(drop=True)
# df.plot(x='Age',y='Proxy')
...
df = pd.read_csv('data/demenocal2000/658C.terr.2.1.interp.csv',
                 header=0,
                 )
df['Age']=df['Age(cal. yr BP)']
df['Proxy']=df['terr% (interp)']
df=df[['Age','Proxy']]
df = df[(df['Age']<=8300)&(df['Age']>=4800)].sort_values('Age',ascending=False).reset_index(drop=True)
# df.plot(x='Age',y='Proxy')
...
df = pd.read_csv('data/deutnat/deutnat.txt',
                 header=85,
...
df=df[['Age','Proxy']]
df = df[(df['Age']<=58000)&(df['Age']>=12000)].sort_values('Age',ascending=False).reset_index(drop=True)
# df.plot(x='Age',y='Proxy')
...
df=df[['Age','Proxy']]
df = df[(df['Age']<=151e3)&(df['Age']>=128e3)].sort_values('Age',ascending=False).reset_index(drop=True)
# df.plot(x='Age',y='Proxy')
